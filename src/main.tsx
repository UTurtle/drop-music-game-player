...
function App() {
  ...
  useEffect(() => { document.documentElement.lang = language; document.title = t('DROP · 내 음악으로 리듬게임', 'DROP · Play your music'); }, [language]);
  ...
  return <>
    ...
    <main>
      {path === '/create' ? <Creator /> : path === '/practice' ? <>
        ...
        <section className="hero"><div className="hero-copy"><span className="eyebrow">{t('내 컴퓨터에서, 내 음악으로', 'YOUR COMPUTER. YOUR MUSIC.')}</span><h1>{t('좋아하는 노래로', 'A song you love.')}<br /><em>{t('한 판 어때요?', 'A game to play.')}</em></h1><p>{t('음악 파일을 고르고, 두 개의 키로 리듬을 타세요.', 'Choose an audio file and follow the rhythm with two keys.')}<br />{t('잘 치지 않아도 괜찮아요. 그냥 재미로.', 'No need to be good. Just have fun.')}</p>
